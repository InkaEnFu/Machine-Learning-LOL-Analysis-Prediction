...
class TestAggregateMeans(unittest.TestCase):
    ...
    def test_means_computed_correctly(self):
        df = pd.DataFrame({
            ...
            'win': [1, 0],
            'role': ['MID', 'MID'],
            'championName': ['Zed', 'Zed'],
            'gameDuration': [1800, 1800],
        })
        result = aggregate_player_games(df)
        self.assertAlmostEqual(result['kda_mean'], 3.0)
        self.assertAlmostEqual(result['kills_mean'], 4.0)
        self.assertAlmostEqual(result['cs_per_min_mean'], 7.0)
